...
export function SongSearch({ defaultValue = "", view = "list", per = DEFAULT_PER_PAGE, songCollection = DEFAULT_COLLECTION, onSelect, className = "" }: Partial<SearchBoxProps> & { onSelect?: (slug: string) => void; className?: string }) {
  ...
  useEffect(() => {
    if (!open || (!query && !numberMode)) return;
  ...
  return (
    <Dialog open={open} onOpenChange={(next) => { setOpen(next); if (next) { setValue(defaultValue); setResults([]); setTotal(0); setError(""); setLoading(Boolean(defaultValue.trim())); setNumberMode(false); } }}>
      ...
      <DialogContent className="z-150 w-[calc(100%-2rem)] max-w-3xl gap-0 overflow-hidden p-0 sm:max-w-3xl">
        ...
        <div className="mx-6 mb-4 flex items-center gap-3 rounded-lg border border-line bg-ground px-4 focus-within:border-brand">
          ...
          <input autoFocus aria-label="Cari lagu atau nomor" inputMode="search" maxLength={150} value={value} onChange={(event) => { const next = event.target.value; setValue(next); setResults([]); setTotal(0); setError(""); setLoading(Boolean(next.trim()) || numberMode); }} placeholder="Nomor, judul, atau lirik…" className="h-11 min-w-0 flex-1 bg-transparent text-base text-ink outline-none" />
          {value && <button type="button" aria-label="Hapus pencarian" className="p-2" onClick={() => { setValue(""); setResults([]); setTotal(0); setError(""); setLoading(numberMode); }}><X size={20} /></button>}
        </div>
        <div className="max-h-[50dvh] min-h-32 overflow-y-auto px-6" aria-busy={loading}>
          {error ? <p role="alert" className="py-4 text-coral">{error}</p> : !query && !numberMode ? <p role="status" className="py-4 text-muted">Ketik judul atau nomor untuk mencari.</p> : !loading && results.length === 0 ? <p role="status" className="py-4 text-muted">Tidak ada lagu yang cocok.</p> : numberMode ? <div className="grid grid-cols-[repeat(auto-fill,minmax(64px,1fr))] gap-2 py-3">{results.map((song) => onSelect ? <button type="button" key={song.slug} className="grid h-14 place-items-center rounded-md border border-line bg-surface font-mono text-sm font-bold text-ink hover:border-brand hover:bg-brand hover:text-white" title={song.title} onClick={() => { onSelect(song.slug); setOpen(false); }}>{song.number || "♪"}</button> : <Link key={song.slug} className="grid h-14 place-items-center rounded-md border border-line bg-surface font-mono text-sm font-bold text-ink hover:border-brand hover:bg-brand hover:text-white" title={song.title} href={`/lagu/${song.slug}?${new URLSearchParams({ ...(query ? { q: query } : {}), ...(view === "nomor" ? { dari: "nomor" } : {}) })}`} onClick={() => setOpen(false)}>{song.number || "♪"}</Link>)}</div> : results.map((song) => {
        ...
        <div className="mt-3 flex items-center justify-between border-t border-line bg-ground px-6 py-4 text-sm text-muted">
          <span role="status">{loading ? "Mencari…" : !query && !numberMode ? "Ketik untuk mencari" : `${total} lagu ditemukan`}</span>
          <span className="flex items-center gap-4">{!onSelect && <Link className="font-semibold text-brand" href={`/?${params}#katalog`} onClick={() => setOpen(false)}>Lihat semua</Link>}<button type="button" className="font-semibold text-brand" onClick={() => { const next = !numberMode; setNumberMode(next); setResults([]); setTotal(0); setError(""); setLoading(next || Boolean(query)); }}>{numberMode ? "Cari berdasarkan judul" : "Pilih berdasarkan nomor"}</button></span>
        </div>
      </DialogContent>
    </Dialog>
  );
